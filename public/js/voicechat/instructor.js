	...
	if(oldTimeDiff > 3){		
		...
		window.location.href = "/instructor/joined-history";
	}else{
		$('.countdiff').val(oldTimeDiff);
		/******************************************************************************/
	...
	function timeIncreaseUp(){
		...
		var y = setInterval(function() {
			...
			if (timeDiff < -1200) {//1200
				...
				$('#timer-bar').html('---End---');
				$('#callStatus').html('End');
				$('#session-finish-div').css('display', 'inherit');
			}
		}, 1000);
	};
		...
		if(id != "0") {
			$.ajax({
				...
				success	: function(res){
					if(flag == 0){
						$('#defaultMsgCountInstructor').val(res.count);
					}else{
						var defaultMsgCount = $('#defaultMsgCountInstructor').val();
						if(res.count > defaultMsgCount){
							$('#messagecountinstructor').html("New");	
							$('#messagecountinstructor').css('display', 'inherit');	
							setTimeout(function(){$('#defaultMsgCountInstructor').val(res.count);}, 10000);
						}else{
							$('#messagecountinstructor').css('display', 'none');	
						}
					}
