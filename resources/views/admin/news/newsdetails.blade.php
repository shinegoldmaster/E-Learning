			...
			<div class="box">
				...
				<div class="box-body">	
					...
					<input name = "newsid" value="{{$data->id}}"  type="hidden" />
						<tr><th  class="text-center" colspan="2">{{trans ('admin.News') }}-{{$data->id}}</th></tr>
						<tr class="text-right">
							<td style="width:250px;">{{trans ('admin.Title') }}</td>
							<td><input type="text" value="{{$data->title}}" name="newstitle"></td>
						</tr>						
						<tr class="text-right">
							<td style="width:250px;">{{trans ('admin.Description') }}</td>
							<td><textarea name="newsdes">{{$data->des}}</textarea></td>
						</tr>
						<tr class="text-right">
							...
							<td>
								...
								<div style= "background-color: aliceblue;" class="col-md-4 col-sm-4 col-xs-12">
									<img class="text-center" src="{{asset($img) }}" style="width:300px"></img>
								</div>
								@endforeach
