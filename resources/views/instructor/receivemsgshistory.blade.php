                        ...
                        <div id="section-10">
							<div class="homework-history">
								...
								<div class="section-title bg-blue">
									<h3>{{trans('instructor.Messages_History')}}</h3>
								</div>
								@if(!$messagehistory)
											...
											<div class="disable-body">
												<div class="display-table">
													<div class="display-cell">
														<p class="text-center">
															{{trans('instructor.There_NO_received_messages')}}
														</p>
													</div>
												</div>
											</div>
								...
								<div class="padding-right-20 padding-left-20 text-center">
									<table class="table table-bordered table-responsive">
										<thead>
											<tr>
												<td>#</td>
												<th>{{trans('instructor.Message_Subject')}}</th>
												<th>{{trans('instructor.Message')}} </th>
												<th>{{trans('instructor.Receiver')}} </th>
										...
										@foreach($messagehistory as $item)
											<tr>
												<td>{{$item->id}}</td>
												<td>{{$item->title}}</td>
												<td><a href="#" class="btn btn-blue" data-toggle="modal" data-target="#msg_{{$item->id}}">{{trans('instructor.Show_Message')}}</a></td>
												<td>{{$item->name}}</td>
												<td><label for="">
														{{$item->created_at}}
													</label>
										...
										<div class="modal-dialog">
											...
											<div class="modal-content">
												<div class="modal-header">
													...
													<h4 class="modal-title">{{trans('instructor.Message')}}</h4>
												</div>
